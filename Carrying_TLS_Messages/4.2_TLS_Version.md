...
在实际操作时，TLS握手会协商出一个要使用的TLS版本。如果两侧终端都支持，那么这可能导致它们最终协商出一个比`1.3`还要高的TLS版本。这是可以接受的，因为QUIC使用的TLS 1.3特性都会受到更高版本的支持。
...
客户端{{< req_level MUST_NOT >}}提供低于`1.3`的TLS版本。未恰当配置的TLS实现可能想要协商TLS 1.2或别的低版本TLS。如果协商了低于`1.3`的TLS版本，终端{{< req_level MUST >}}终止连接。
